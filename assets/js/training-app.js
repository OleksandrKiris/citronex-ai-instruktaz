(() => {
  ...
  function renderHomeCompact() {
    ...
    ` : "";
    const guideLabels = welcomeGuideUi[lang] || welcomeGuideUi.pl;
    const guideSentences = guideText(lang, getLocationName());
    const presenterTitle = text(tx(
      "Aleksandr — cyfrowy przewodnik",
      "Aleksandr — digital guide",
      "Олександр — цифровий провідник",
      "Александр — цифровой помощник",
      "Aleksandr — rəqəmsal bələdçi",
      "Aleksandr — guía digital",
      "Aleksandr — digital na gabay",
      "Aleksandr — pemandu digital",
      "अलेक्जेन्डर — डिजिटल मार्गदर्शक"
    ));
    const presenterBadge = text(tx("Przed rozpoczęciem", "Before you start", "Перед початком", "Перед началом", "Başlamazdan əvvəl", "Antes de empezar", "Bago magsimula", "Sebelum mulai", "सुरु गर्नु अघि"));
    const presenterNote = text(tx(
    ...
    const presenter = `
      <section class="presenter-card" data-presenter-card aria-labelledby="presenterTitle">
        <div class="presenter-media">
          <img src="assets/brand/digital-presenter.png" alt="${esc(presenterTitle)}" width="1536" height="1024">
          <span class="presenter-speaking" aria-hidden="true"><span></span><span></span><span></span></span>
        </div>
        <div class="presenter-content">
          <p class="presenter-badge">${esc(presenterBadge)}</p>
          <h2 id="presenterTitle">${esc(presenterTitle)}</h2>
          <p class="presenter-script" data-presenter-script aria-live="polite">${esc(guideSentences.join(" "))}</p>
          <p class="presenter-note">${esc(presenterNote)}</p>
          <div class="presenter-actions">
            <button type="button" class="btn" data-presenter-play>${esc(guideLabels.start)}</button>
            <button type="button" class="btn secondary" data-presenter-stop hidden>${esc(tx("Zatrzymaj", "Stop", "Зупинити", "Остановить", "Dayandır", "Detener", "Ihinto", "Hentikan", "रोक्नुहोस्")[lang] || "Stop")}</button>
            <a class="btn secondary" href="${esc(href("mapa"))}">${esc(text(tx("Najpierw: droga i mapa", "First: route and map", "Спочатку: дорога і карта", "Сначала: дорога и карта", "Əvvəlcə: yol və xəritə", "Primero: ruta y mapa", "Una: daan at mapa", "Pertama: rute dan peta", "पहिले: बाटो र नक्सा")))}</a>
          </div>
        </div>
      </section>
    `;
    app.innerHTML = `<main class="page">${pageHero("home")}${presenter}<section class="tiles">${primaryTiles.map(renderTile).join("")}${moreTile}</section></main>`;
  }
